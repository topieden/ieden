...
public class TypeTemplateServiceImpl implements TypeTemplateService {
	...
	private void saveToRedis(){
		//为什么要放到这里呢??
		//缓存品牌和规格数据  先有一个大key品牌列表 将模板id作为小key,将品牌列表作为值
		
		
		//获取模板数据
		List<TbTypeTemplate> typeTemplatesList = findAll();
		//循环模板
		for (TbTypeTemplate tbTypeTemplate : typeTemplatesList) {
			//存储品牌列表
	...
	public PageResult findPage(TbTypeTemplate typeTemplate, int pageNum, int pageSize) {
		...
		public List<Map> findSpecListById(long id) {
			...
			for (Map map : specList) {
				...
				List<TbSpecificationOption> optionsList = specificationOptionMapper.selectByExample(example);
